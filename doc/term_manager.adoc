...
<4> This instance has chosen a weird subdomain that doesn't match its hosted site name, so we need to provide the `uri`.
...
<6> BEST CASE! The site name in the parameter name is the subdomain of the instance, and we'll use the standard admin username and fetched password.
