...
RSpec.describe CollectionspaceMigrationTools::ConfigSubdirectoryHandler do
  ...
  let(:config){ Struct.new(:base_dir, :mapper_dir).new(base_dir_val, mapper_dir_val) }
  ...
  context 'with absolute path given' do
    context 'when given dir exists' do
      ...
      it 'does not change config', :aggregate_failures do
        ...
        expect(Dir.exists?(mapper_dir_val)).to be true
      end
    end
    ...
    context 'when given dir does not exist' do
      ...
      it 'raises error' do
        expect{ result }.to raise_error(CMT::ConfigSubdirectoryHandler::NonExistentDirectorySpecifiedError)
      end
    end
  end
  ...
  context 'when relative path given' do
    ...
    context 'when given dir exists' do
      ...
      it 'updates config to absolute path', :aggregate_failures do
        ...
        expect(Dir.exists?(File.expand_path(mapper_dir_val))).to be true
      end
    end
  ...
  context 'when subdir given' do
    ...
    context 'when given dir does not exist' do
      ...
      after{ FileUtils.rm_rf(@path) }
      it 'updates config to absolute path and creates directory', :aggregate_failures do
        result
        expect(config.mapper_dir).to eq(@path)
