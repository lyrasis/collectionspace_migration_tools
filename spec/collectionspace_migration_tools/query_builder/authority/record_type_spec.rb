...
RSpec.describe CollectionspaceMigrationTools::QueryBuilder::Authority::RecordType do
  describe '.initialize' do
    ...
    context 'with unknown rectype' do
      it 'raises UnknownTypeError' do
        errclass = CMT::QB::Authority::UnknownTypeError
  ...
  describe '#service_type' do
    let(:result){ described_class.new(rectype).service_type }
  ...
  describe '#term_table' do
    let(:result){ described_class.new(rectype).term_table }
    let(:rectype){ 'citation' }
  ...
  describe '#term_group_table' do
    let(:result){ described_class.new(rectype).term_group_table }
    let(:rectype){ 'citation' }
  ...
  describe '#vocab_table' do
    let(:result){ described_class.new(rectype).vocab_table }
    let(:rectype){ 'citation' }
