...
RSpec.describe CollectionspaceMigrationTools::Batch::IngestStatusChecker do
  ...
  let(:checks){ 1 }
  let(:rechecks){ 1 }
  let(:klass){ described_class.new(lister: lister, wait: 0.1, checks: checks, rechecks: rechecks) }
    ...
    context 'with single check' do
      context 'when no objects left on check' do
        it 'behaves as expected' do
          ...
          allow(klass).to receive(:get_size).exactly(sizes.length).times{ Dry::Monads::Success(sizes.shift) }
          expect(result).to be_a(Dry::Monads::Success)
        end
      end
