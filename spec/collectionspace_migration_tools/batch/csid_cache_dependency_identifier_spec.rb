...
RSpec.describe CollectionspaceMigrationTools::Batch::CsidCacheDependencyIdentifier do
  ...
  describe '#call' do
    ...
    context 'when authorityhierarchy' do
      let(:path){ File.join(Bundler.root, 'spec', 'support', 'fixtures', 'ah_csid_dep_test.csv') }
      let(:mapper){ CMT::Parse::RecordMapper.call('authorityhierarchy').value! }
    ...
    context 'when nonhierarchicalrelationship' do
      let(:path){ File.join(Bundler.root, 'spec', 'support', 'fixtures', 'nhr_csid_dep_test.csv') }
      let(:mapper){ CMT::Parse::RecordMapper.call('nonhierarchicalrelationship').value! }
