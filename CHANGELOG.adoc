...
Switched from using releases to tags with 7.3.0.
Old releases were deleted from the repository, leaving their associated tags in place.
This was done to avoid v7.2.0 displaying as "latest release" on the repo front page forever.

The top-level headers in this file should match the tags for the given changes.

Tags previously associated with releases are prefixed with "v", as the tags were generally aligned with the release names including the prefix.

Going forward, tags will omit the "v".

The tags generally align with the spirit of https://semver.org[semantic versioning].

== Untagged

=== Added

* Count-only `thor bucket objct` and `thor batch lbc` commands.
NOTE: They are still sloooow because S3 has to return all the object keys
* Progress bar to `thor csid delete` command
* `thor vt delete_all VOCAB` command
* `thor rels check_nhrs` command

=== Bugfixes

* https://github.com/lyrasis/collectionspace_migration_tools/issues/139[#139] - Fixed by checking whether there are any AWS log events for the batch before declaring the batch complete and attempting to find the time of the latest log event for the batch in order set the batch ingest completion time and duration.

== 7.3.0 (2026-08-05)

* Add `path` option to `thor rr list` command
* Add progress bar to `thor batch upload` command
* Add new `db_tunnel_initial_connection_timeout` and `db_tunnel_initial_connection_polling_interval` system config settings. See `sample_system_config.yml` for description.
* Accept CollectionSpace QA profile version values in client config
* Add `thor vt write` command
* Require Ruby 4.0.3
* DEV: Remove unused/unneeded dependencies; minor refactoring
...
- Fix `thor config show` error described in https://github.com/lyrasis/collectionspace_migration_tools/issues/113[#113]
...
* Fixed the occasional spurious duplicate file error described in https://github.com/lyrasis/collectionspace_migration_tools/issues/69[#69]
...
* Do not hit CHIA if getting non-verbose current client config (Resolves https://github.com/lyrasis/collectionspace_migration_tools/issues/73[#73])
